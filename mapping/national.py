...
def create_category_uri(g, data):
  for category in data['Classification'].unique():
    category_id = concept_uri(ns.oc, category)
...
def main(file):
  ...
  for index, row in national_raw.iterrows():
    abb_id = concept_uri(ns.lblod + 'organisatie/', str(row['representatief orgaan']))

    g.add((abb_id, RDF.type, ns.org.Organization))

    add_literal(g, abb_id, SKOS.prefLabel, str(row['representatief orgaan']), XSD.string)

    g.add((abb_id, ns.org.classification, concept_uri(ns.oc, row['Classification'])))

    site_id = concept_uri(ns.lblod + 'vesting/', str(row['representatief orgaan']))
    g.add((site_id, RDF.type, ns.org.Site))

    contact_id = concept_uri(ns.lblod + 'contactpunt/', str(row['representatief orgaan']))
    ...
    address_id = concept_uri(ns.lblod + 'adresvoorstelling/', str(row['representatief orgaan']))
    ...
    bestuur_temporary = concept_uri(ns.lblod + 'bestuursorgaan/', (row['representatief orgaan'] + str(datetime.datetime.now())))
    ...
    person_id = concept_uri(ns.lblod + 'persoon/', str(row['Gebruikte Voornaam']) + str(row['Achternaam']))
    ...
    ## Mandaat
    rol_mandaat = concept_uri(ns.lblod + 'mandaat/', str(row['representatief orgaan']) + str(row['Gebruikte Voornaam']) + str(row['Achternaam']))
    ...
    ## Mandataris
    mandataris = concept_uri(ns.lblod + 'mandataris/', str(row['representatief orgaan']) + str(row['Gebruikte Voornaam']) + str(row['Achternaam']) + str(row['Rol'].lower().replace(" ", "")))
    ...
    g.add((person_id, ns.mandaat.isAangesteldAls, ns.person_mandataris))
