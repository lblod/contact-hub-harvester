...
def main(file, mode):
  ...
  for _, row in national_raw.iterrows():
    ...
    kbo_uri, _  = concept_uri(lblod + 'gestructureerdeIdentificator/', str(row['KBO_nr']))
    ...
    site_id, _ = concept_uri(lblod + 'vestiging/', str(row['representatief orgaan']))
    ...
    address_id, _ = concept_uri(lblod + 'adres/', str(row['representatief orgaan']))
