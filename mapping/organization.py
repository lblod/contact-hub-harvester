...
def main(file, mode): 
  ...
  for _, row in orgs_cleansed.iterrows():
    ...
    abb_id, abb_uuid = concept_uri(lblod + 'bestuurseenheid/', str(row['organisation_id']))
    ...
    if str(row['KBOnr_cleansed']) != str(np.nan):
      ...
      kbo_id, _ = concept_uri(lblod + 'gestructureerdeIdentificator/', str(row['organisation_id']) + str(row['KBOnr_cleansed']))
    ...
    if str(row['Unieke Naam']) != str(np.nan):
      unieke_naam_id, _ = concept_uri(lblod + 'gestructureerdeIdentificator/', str(row['organisation_id']) + str(row['Unieke Naam']) + '1')
    ...
    if exists_site_org(row):
      ...
      if exists_contact_org(row):
        contact_id, _ = concept_uri(lblod + 'contactinfo/', str(row['organisation_id']))
      ...
      if exists_address(row):
        ...
        g.add((address_id, ns.locn.address, address_id))
      
      g.add((abb_id, ns.org.hasPrimarySite, site_id))
      g.add((abb_id, ns.org.hasSite, site_id)) #?

      g.add((site_id, ns.org.hasSite, abb_id))
    if row['Unieke Naam'] != row['Moederentiteit']:
      ...
      if len(find_moeder_kboid) > 0:
        org_id, _ = concept_uri(lblod + 'organisatie/', str(find_moeder_kboid.iloc[0]['organisation_id']))
        g.add((abb_id, ns.org.linkedTo, org_id))

    if pd.notna(row['Actief vanaf']):
      change_event_open_id, _ = concept_uri(lblod + 'veranderingsgebeurtenis/', str(row['organisation_id']) + str(row['Actief vanaf']))
    ...
    if row['Organisatiestatus'] == 'Valt niet meer onder Vlaams toezicht':
      change_event_not_flemish_id, _ = concept_uri(lblod + 'veranderingsgebeurtenis/', str(row['organisation_id']) + str(row['Actief tot']) + str(row['Actief tot']))
      ...
      change_event_merged_id, _ = concept_uri(lblod + 'veranderingsgebeurtenis/', str(row['organisation_id']) + str(row['Actief tot']))
      ...
      merged_abb_id, _ = concept_uri(lblod + 'organisatie/', str(row['Resulting organisation']))
      ...
      change_event_close_id, _ = concept_uri(lblod + 'veranderingsgebeurtenis/', str(row['organisation_id']) + str(row['Actief tot']))
