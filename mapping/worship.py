...
from helper.functions import add_literal, concept_uri, export_data, export_df
...
def main(file): 
  ...
  for _, row in worship_cleansed.iterrows():
    ...
    address_id, _ = concept_uri(ns.lblod + 'adresvoorstelling/', str(row['organization_id']))
    ...
    # Voorzitter
    voorzitter, _ = concept_uri(ns.lblod + 'persoon/', str(row['Naam_voorzitter_EB First']) + str(row['Naam_voorzitter_EB Last']))
    ...
    ## Voorzitter - Vesting
    voorzitter_vesting_uri, _ = concept_uri(ns.lblod + 'vestiging/', str(row['organization_id']) + str(row['Naam_voorzitter_EB First']) + str(row['Naam_voorzitter_EB Last']))
    ...
    voorzitter_contact_uri, _ = concept_uri(ns.lblod + 'contactpunt/', str(row['organization_id']) + str(row['Naam_voorzitter_EB First']) + str(row['Naam_voorzitter_EB Last']) + str(row['Tel_voorzitter_EB 1']))
    ...
    add_literal(g, voorzitter_contact_uri, ns.schema.telephone, str(row['Tel_voorzitter_EB 1']), XSD.string)
    add_literal(g, voorzitter_contact_uri, ns.schema.email, str(row['Mail_voorzitter_EB Cleansed']), XSD.string)  

    if str(row['Tel_voorzitter_EB 2']) != str(np.nan):
      voorzitter_contact_2_uri, _ = concept_uri(ns.lblod + 'contactpunt/', str(row['organization_id']) + str(row['Naam_voorzitter_EB First']) + str(row['Naam_voorzitter_EB Last']) + str(row['Tel_voorzitter_EB 2']))
      ...
      add_literal(g, voorzitter_contact_2_uri, ns.schema.telephone, str(row['Tel_voorzitter_EB 2']), XSD.string)

    ### Voorzitter - Adres
    voorzitter_address_id, _ = concept_uri(ns.lblod + 'adresvoorstelling/', str(row['organization_id']) + str(row['Naam_voorzitter_EB First']) + str(row['Naam_voorzitter_EB Last'])) 
    ...
    add_literal(g, voorzitter_address_id, ns.locn.fullAddress, str(row['Adres_voorzitter_EB Cleansed']))
    
    ## Voorzitter - Mandaat
    voorzitter_mandaat, _ = concept_uri(ns.lblod + 'mandaat/', str(row['organization_id']) + str(row['Naam_voorzitter_EB First']) + str(row['Naam_voorzitter_EB Last']))
    ...
    voorzitter_mandataris, _ = concept_uri(ns.lblod + 'mandataris/', str(row['organization_id']) + str(row['Naam_voorzitter_EB First']) + str(row['Naam_voorzitter_EB Last']) + 'voorzitter')
